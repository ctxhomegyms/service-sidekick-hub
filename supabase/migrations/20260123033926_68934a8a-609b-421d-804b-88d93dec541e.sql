-- Create global search function using full-text search
...
BEGIN
  ...
  SELECT 
    'message'::text,
    m.conversation_id,
    COALESCE(c.name, 'Unknown Contact'),
    'Message',
    ts_headline('english', m.content, plainto_tsquery('english', search_term), 'MaxWords=20, MinWords=5')::text,
    ts_rank(to_tsvector('english', m.content), plainto_tsquery('english', search_term))::float
  FROM messages m
